            ...
            if mode == "v1":
                ...
                for m in matches:
                    if m in self.characters:
                        pattern = r"(\S+) leave the conversation."
...
class DramaLLM(World):
    ...
    def v2_react(self):
        ...
        self.nc = response["Chain"]
        for char_id in self.characters:
            if char_id == response["Next Action Character"]:
                self.characters[char_id].to_do = True
                self.characters[char_id].motivation = response["Action Character's Instruction"]
